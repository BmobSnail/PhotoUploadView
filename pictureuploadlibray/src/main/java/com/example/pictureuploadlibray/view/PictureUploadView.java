...
import com.example.pictureuploadlibray.constant.PathData;
...
/**
 * Created by Snail小江 on 2016/7/22.
 * 图片选择器
 */
public class PictureUploadView extends GridView implements UploadPicHelper.SelectPicListener {
    ...
    public PictureUploadView(Context context, AttributeSet attrs) {
        ...
        options = new DisplayImageOptions.Builder()
                .showImageForEmptyUri(R.color.half_transparent)//设置图片Uri为空或是错误的时候显示的图片
                .showImageOnFail(R.color.half_transparent)  //设置图片加载/解码过程中错误时候显示的图片
            ...
            switch (type) {
                ...
                case COMMON_PIC:
                    if (mPaths.get(position).path.contains("http://")) {
                        ImageLoader.getInstance().displayImage(mPaths.get(position).path, viewHolder.image, options);
                        viewHolder.cancel.setVisibility(VISIBLE);
                        viewHolder.resend.setVisibility(GONE);
                        viewHolder.progress.setVisibility(GONE);
                    } else {
                        ImageLoader.getInstance().displayImage("file://" + mPaths.get(position).path, viewHolder.image, options);
                    }
                    listener.setIds(0);
    ...
    public HashMap<String, ProgressBar> getAllProgressView() {
        for (String key : mAllProgressBarMap.keySet()) {
            Log.i("upload test", "path:" + key);
        }
        return mAllProgressBarMap;
    }

    public HashMap<String, ImageView> getAllResendView() {
        for (String key : mAllResendViewMap.keySet()) {
            Log.i("upload test", "path:" + key);
        }
